...
pure elemental real*8 function cox_voinov_lambda1(theta_rad) result(G)
  implicit none
  ...
  G  = c0 + a1*x + a3*x*x*x
end function cox_voinov_lambda1


pure elemental real*8 function dcox_voinov_dtheta_lambda1(theta_rad) result(dG)
  implicit none
  real*8, intent(in) :: theta_rad
  real*8 :: pi, x
  real*8, parameter :: a1 = 0.230884833059d0
  ...
  x  = theta_rad - 0.5d0*pi
  dG = a1 + 3.0d0*a3*x*x
end function dcox_voinov_dtheta_lambda1
...
!  - tol, itmax: 収束条件
pure elemental real*8 function cox_voinov_inverse_lambda1(theta_init, g_target,           &
  ...
  do it = 1, itmax
    err = cox_voinov_lambda1(theta_sol) - g_target
    if (abs(err) < tol) exit
    theta_new = theta_sol - err / dcox_voinov_dtheta_lambda1(theta_sol)
...
end function cox_voinov_inverse_lambda1
