      subroutine trans_r2w(ipara,ID,ndiv,svall,key,sendbuf,recvbuf,r,w)
      implicit none
      include 'mpif.h'
...
!$OMP$ SHARED(sendbuf,recvbuf,r)
      do k=          1,svall(3)/ndiv
      do j=          1,svall(2)/ndiv
      do i=-svall(1)/2,svall(1)/2-1
      kk=k+l*svall(3)/ndiv-svall(3)/2-1
      sendbuf(i,j,k,l)=r(i,j,kk)
      recvbuf(i,j,k,l)=r(i,j,kk)
      enddo
      enddo
      enddo
!$OMP END PARALLEL DO
...
!$OMP$ SHARED(l,ndiv,svall)
!$OMP$ SHARED(w,recvbuf)
      do k=          1,svall(3)/ndiv
      do j=          1,svall(2)/ndiv
      do i=-svall(1)/2,svall(1)/2-1
      jj=j+l*svall(2)/ndiv
      w(i,jj,k)=recvbuf(i,j,k,l)
      enddo
      enddo
      enddo
!$OMP END PARALLEL DO
